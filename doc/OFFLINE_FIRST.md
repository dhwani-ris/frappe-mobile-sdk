...
3. Each upload is `POST /api/method/upload_file` with **only** `file`, `file_name`, `is_private`. **No** `dt`, **no** `dn` — Frappe v16's File controller rejects partial-attach uploads (where `attached_to_doctype` is set but `attached_to_name` is empty), so the upload must be fully unattached.
...
There is no 60-minute cap (Frappe's `relink_mismatched_files` flow has one; this design doesn't use it). An offline retry that lands hours or days later still relinks correctly because the match key is `file_url`, not a temp-name window.
