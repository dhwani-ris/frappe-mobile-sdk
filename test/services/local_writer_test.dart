...
void main() {
  ...
  test(
    'writes parent + splits Table children into per-doctype tables',
    () async {
      ...
      expect(
        parentRows.first['customer__is_local'],
        0,
  ...
  test('markSynced sets server_name + sync_status=synced', () async {
    await writer.writeParent(
      parentDoctype: 'Order',
      data: {'mobile_uuid': 'p-uuid-3', 'title': 'pending push'},
    );
  ...
  test('writeParent with serverName + syncOp keeps sync_status=dirty '
      '(user-driven offline edit of a synced doc)', () async {
    await writer.writeParent(
      parentDoctype: 'Order',
      data: {'mobile_uuid': 'p-uuid-edit', 'title': 'edited title'},
      serverName: 'ORD-00001',
      syncOp: 'UPDATE',
    );
    ...
    final rows = await db.query(
      'docs__order',
      where: 'mobile_uuid = ?',
      whereArgs: ['p-uuid-edit'],
    );
    expect(
      rows.first['server_name'],
      'ORD-00001',
      reason: 'server_name must be preserved',
    );
    expect(
      rows.first['sync_status'],
      'dirty',
      reason: 'user edit must be dirty even though serverName is known',
    );
    expect(rows.first['sync_op'], 'UPDATE');
  });

  test('writeParent with serverName and no syncOp marks row synced '
      '(applyServerDocument path)', () async {
    await writer.writeParent(
      parentDoctype: 'Order',
      data: {'mobile_uuid': 'p-uuid-pull', 'title': 'from server'},
      serverName: 'ORD-00002',
      // syncOp intentionally omitted — simulates PullApply / applyServerDocument
    );

    final rows = await db.query(
      'docs__order',
      where: 'mobile_uuid = ?',
      whereArgs: ['p-uuid-pull'],
    );
    expect(rows.first['server_name'], 'ORD-00002');
    expect(
      rows.first['sync_status'],
      'synced',
      reason: 'server-apply without syncOp must remain synced',
    );
  });

  test(
    'no-op when parent table not yet created (initial sync pending)',
    () async {
